...
{
    gchar *choix = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(widget)); //On récupère la valeur choisie dans la liste déroulante chosie
    ...
    mode = choix; // on met la valeut chosie dans mode
    g_free(choix); 
...
void on_lancer_button(GtkButton *button, gpointer user_data) //Fonction callback execute lors du signal "clicked"
{
    ...
    {
        gchar *couleur = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(menu_couleur)); //on prend la couleur sélectionné
        
        if (couleur != NULL)
        {
            FILE *file = fopen("couleur.txt", "w");
            if(file != NULL)
            {
                fprintf(file, "%s\n", couleur);
                fclose(file);
    ...
    {
        gchar *teeshirt = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(menu_teeshirt)); //sélection du teeshirt récupéré + masque
        gchar *masque = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(menu_masque));
...
{ 
    ...
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(menu_teeshirt), "bleu");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(menu_teeshirt), "noir");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(menu_teeshirt), "blanc");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(menu_teeshirt), "gris");
    gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(menu_teeshirt), "orange");
